...
This builds a **hand-coded React marketing/landing site** from a fixed premium template. Use it for exactly that. For other jobs, hand off:
- **A store** (products, cart, checkout) → `cms-website-builder` (Shopify/WordPress/Wix) — don't force e-commerce onto this template.
- **An app / dashboard / client portal** with auth and real backend data → `web-app-development`.
- **A one-off bespoke component or a site that must look unlike anything else** → `frontend-design` (this skill deliberately reskins one proven template fast; it's not the tool when total visual uniqueness is the goal).

If the site is for an established brand, pull its actual color/type tokens from `brand-guidelines` (for DVC's own) or `brand-developer` (for a client's) instead of asking for colors cold in intake.
...
- **Stack is pinned to Tailwind v3 on purpose** — the entire bundled reference (`tailwind.config.js`, `postcss.config.js`, the `@tailwind` directives in `index.css`) is built on v3's config model. Don't half-upgrade to v4 mid-build; it changes the config model and will break the reference. A full v4 modernization is a separate, deliberate rewrite of all the reference files, not an in-build tweak.
- **Font stack still uses Inter for body** — fine inside this cohesive premium system, but note it's on `frontend-design`'s "generic/AI-slop" list; reconcile during the Tailwind-v4 modernization, not piecemeal.
- **Don't skip the re-skin** — the single most common failure is shipping the reference's water-drop animation (or its plumbing/Danish copy) on a business it doesn't belong to. Every string and the signature animation must be translated/re-skinned to the target.
