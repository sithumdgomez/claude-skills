...
1. **Invoke it as a sub-skill** (via the Skill tool, name `scalability`) when you want a bottleneck/scaling review of a specific design (a query, an API, a data model) before finalizing it — pass the design in question and get back concrete findings (bottleneck + fix), not a standalone report.
2. **Read the relevant reference file directly** (e.g. `database-scaling.md`, `caching-and-queues.md`) when you just need a quick pattern lookup — the right index shape, a caching pattern — without a full review pass.
...
When invoked by another skill mid-build, return findings concisely (bottleneck + fix, per the decision matrix above) so the calling skill can act on them directly, rather than writing a long standalone report.
