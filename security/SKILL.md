...
1. **Invoke it as a sub-skill** (via the Skill tool, name `security`) when you want a reviewed pass on specific code, an API design, or a feature before finalizing it — pass the code/design/area in question and get back concrete findings (severity + fix), not a standalone report.
2. **Read the relevant reference file directly** (e.g. `web-security.md`, `auth-and-secrets.md`) when you just need a quick pattern lookup — the correct CSP header, the right password-hashing call — without a full review pass.
...
When invoked by another skill mid-build, return findings concisely (severity + fix, per the table above) so the calling skill can act on them directly, rather than writing a long standalone report.
